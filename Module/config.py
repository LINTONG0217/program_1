...
# --- 感知模块配置 ---
# 当前工程已精简为：仅保留 YDLIDAR T-MINI PLUS 雷达感知。
...
# 启动扫描：部分 YDLIDAR（含 T-MINI PLUS）需要主控发送指令后才开始持续输出 AA 55 扫描包。
# - STOP: A5 65
# - SCAN: A5 60
...
# UART 兼容/探测：
# - 在 sys.platform == "mimxrt"（常见 OpenMV/OpenART 风格固件）上，UART 往往不支持传 tx/rx，且 UART 号可能不是 5。
# - 开启 AUTO_FALLBACK 后会尝试多个 UART id，并通过 PROBE_MS 选取“真的在收数据”的那一路。
LIDAR_UART_AUTO_FALLBACK = True
LIDAR_UART_PROBE_MS = 150

# 雷达角度窗口（单位：度，前方为 0，左正右负）
LIDAR_FRONT_MIN_DEG = -75
LIDAR_FRONT_MAX_DEG = 75
...
# 坐标系建议：以场地某一角为(0,0)，x沿3.2m方向，y沿2.4m方向，yaw为朝向x轴的角度(度)。
FIELD_SIZE_X_M = 3.2
FIELD_SIZE_Y_M = 2.4
...
# lidar 模式建议关闭该开关，以启用 CompetitionController 的“无 zone 直推”逻辑。
# 注意：由于 mimxrt 固件内存极其受限，强行加载全套 Controller 会导致 76KB alloc MemoryError。
