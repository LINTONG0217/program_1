"""跟随测试（competition.py 同结构：build_chassis/build_vision）。

用途：
- 复用工程的 Motor/OmniChassis/VisionReceiver 初始化方式
- 读取 vision 的 object（offset_x/size/held）并输出底盘 vx/vy/vw
- 在终端限频打印视觉目标与链路统计，方便确认“确实收到识别结果”

运行：在主控 REPL 执行：
    exec(open('tests/follow_competition_style.py').read())
"""
...
def wait_c14_start():
    ...
    try:
        ...
        print("press C14 to start follow")
...
def main():
    print("follow_competition_style start")
    ...
    while True:
        ...
        if time.ticks_diff(now, last_print) >= print_ms:
            ...
            try:
                ...
                pend = vision.rx_pending() if hasattr(vision, "rx_pending") else None
            except Exception:
                stat = None
                age = None
                pend = None
            ...
            if obj:
                ...
                print("VISION: no object", "age_ms=", age, "pend=", pend, "stat=", stat)

        if not obj:
            search_speed = getattr(config, "SEARCH_ROT_SPEED", 18)
            chassis.rotate(int(search_speed * 0.5))
            time.sleep_ms(int(getattr(config, "LOOP_DELAY_MS", 50)))
            continue
